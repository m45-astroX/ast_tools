...
#define ENERGY_DENSITY 100              // 陽子のエネルギー密度(eV cm**(-3))
#define PROTON_MASS 938.272             // 陽子の静止質量(MeV)
#define PROTON_INDEX 2.5                // 陽子の冪指数
#define PROTON_VELOCITY 10.0            // 陽子の速度(MeV)
...
int main ( int argc, char *argv[] ) {
    ...
    if ( argc != 6 ) {
        fprintf(stderr, "Usage : ./calc_line_flux_proton cross-section omega relative_abundance integ_min(keV) integ_max(keV)\n");
...
double calc_K ( double integ_min, double integ_max ) {
    ...
    return ( ENERGY_DENSITY * 0.001 ) / ( ( 1.0 / ( 2.0 - PROTON_INDEX ) ) * ( pow(integ_max, 2.0-PROTON_INDEX) - pow(integ_min, 2.0-PROTON_INDEX) ) );
...
double calc_n ( double integ_min, double integ_max ) {
    ...
    return K * ( 1 / ( 1 - PROTON_INDEX ) ) * ( pow(integ_max, 1-PROTON_INDEX) - pow(integ_min, 1-PROTON_INDEX) );
...
double calc_velocity ( void ) {
    ...
    return VELOCITY_LIGHT * sqrt( 1-(1/pow(1+PROTON_VELOCITY/PROTON_MASS, 2)) );
...
double calc_sky_omega ( void ) {
    ...
    return 2.0 * M_PI * ( 1.0 - cos(radius_rad) );
