...
//#define DEBUG
// フィットする範囲の指定
#define FIT_MIN_KEV 6.0
#define FIT_MAX_KEV 6.5
// 観測スペクトルの情報
...
// factor
#define FACTOR_INIT 0.01
#define FACTOR_MAX  5.0
#define FACTOR_STEP 0.05
...
int main ( int argc, char *argv[] ) {
    ...
    while ( fgets( buf, sizeof(buf), fp_obsspec ) != NULL ) {
        ...
        //    $1(0) : Energy (keV)
        //    $2(1) : Counts (counts s^-1 keV^-1)
        if( sscanf( buf, "%lf %lf", &readvalue[0], &readvalue[1] ) != 2 ) {
            continue;
        }
    ...
    for ( double factor=FACTOR_INIT; factor<=FACTOR_MAX; factor=factor+FACTOR_STEP) {
        ...
        for ( int i=RESOLVE_MIN_CH; i<=RESOLVE_MAX_CH; i++ ) {
            ...
            chiSquare += ( obs_spec[i] - sim_spec[i]*factor ) * ( obs_spec[i] - sim_spec[i]*factor );
