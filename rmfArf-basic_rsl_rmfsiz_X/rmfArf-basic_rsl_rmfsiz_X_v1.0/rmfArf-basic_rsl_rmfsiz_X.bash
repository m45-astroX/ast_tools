...
# 2025.03.20 v1.0 Yuma Aoki (Kindai Univ.)
#   - whichrmf='X' オプションでRMFを作成するスクリプト
#   - スクリプトはrmfArf-basic_rsl_v3.0をベースとした
#   - regionファイルをDEFAULTに設定した場合、
#     Pixel 12と27を除外したregionファイルが使用される
...
if [ $# -eq 7 ] || [ $# -eq 8 ] ; then
    ...
    if [ "$f_region_read" = 'DEFAULT' ] ; then
        f_region="$(cd $(dirname $0) && pwd)/regionfile/region_RSL_det_wo_px_12_27.reg"
    else
    ...
    if [ "$source_type_read" = 'FLATCIRCLE' ] || [ "$source_type_read" = 'flatcircle' ] || [ "$source_type_read" = 'FLAT' ] || [ "$source_type_read" = 'flat' ] || [ "$source_type_read" = 'F' ] || [ "$source_type_read" = 'f' ] ; then
        ...
        if [ $# -ne 8 ] ; then
            echo "*** ERROR"
            echo "When sourcetype is set to 'FLATCIRCLE', flatradius (\$8) is requierd."
            echo "abort"
            exit
        fi
        ...
        echo "*** ERROR"
        echo "Source type (\$7) must be 'FLATCIRCLE' or 'POINT'!"
        echo "abort"
        exit
    fi
    ...
    echo ""
    echo "Usage : bash rmfArf-basic_rsl.bash"
    echo "    \$1 BASENAME"
    ...
    echo "          regionfile/region_RSL_det_wo_px_12_27.reg will be used."
...
### Variables ###
rmf_whichrmf='X'    # S, M, L or X
expmap_delta=20.0
expmap_numphi=1
arf_emin=0.3
arf_emax=30.0
arf_numphoton=1000000
arf_minphoton=100
...
calcCenter4rsl="${d_bin}/calcCenter4rsl/calcCenter4rsl"


### Directories ###
d_log="./logfiles_rmfArf-basic_rsl_rmfsiz_X"
f_log="${d_log}/rmfArf-basic_rsl_rmfsiz_X.log"
if [ -d $d_log ] ; then
    rm -rf $d_log
else
    mkdir $d_log
...
### Make ###
make clean -C ${d_bin}/calcCenter4rsl 1>> $f_log 2>> $f_log
status=$?; if [ $status != 0 ] ; then echo "make clean error (status = $status)"; echo "abort."; exit; fi
make -C ${d_bin}/calcCenter4rsl 1>> $f_log 2>> $f_log
...
echo "arf_emin      = ${arf_emin}" >> $f_log
echo "arf_emax      = ${arf_emax}" >> $f_log
...
rslmkrmf \
    ...
    eminin=0.0 dein=0.5 nchanin=60000 useingrd=no \
    1>> $f_log 2>> $f_log
