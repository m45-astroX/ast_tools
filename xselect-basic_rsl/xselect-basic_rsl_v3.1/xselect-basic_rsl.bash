...
# 2025.02.21 v3.0 Yuma Aoki (Kindai Univ.)
#   - clean2 のスクリーニング条件を修正
# 2025.06.05 v3.1 Yuma Aoki (Kindai Univ.)
#   - xselect の仕様変更に対応 (V2.5c)
#   - PIファイルの作成時における以下のコマンドを削除した
#       - $ filter pha_cutoff $IMG_PHAMIN $IMG_PHAMAX
#       - 実行しない場合、PIファイルのチャンネル数が$IMG_PHAMIN $IMG_PHAMAXになってしまう
...
if [ $# -eq 2 ] ; then
    ...
    echo ""
    echo "Usage : bash xselect-basic_rsl.bash"
...
# Create clean2 event
echo "" >> $f_log
echo "CMD : ftcopy infile=${f_evt_fullpath}[EVENTS][(PI>=600) && (((((RISE_TIME+0.00075*DERIV_MAX)>46)&&((RISE_TIME+0.00075*DERIV_MAX)<58))&&ITYPE<4)||(ITYPE==4))&&STATUS[4]==b0] outfile=${f_evt_clean2_dir}/${f_evt_clean2} copyall=yes clobber=yes history=yes" >> $f_log
echo "" >> $f_log
ftcopy \
    infile="${f_evt_fullpath}[EVENTS][(PI>=600) && (((((RISE_TIME+0.00075*DERIV_MAX)>46)&&((RISE_TIME+0.00075*DERIV_MAX)<58))&&ITYPE<4)||(ITYPE==4))&&STATUS[4]==b0]" \
...
# Zip files
gzip $f_evt_clean2_dir/$f_evt_clean2
