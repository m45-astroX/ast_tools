...
if [ $# -eq 2 ] ; then
    ...
    if [ "$3" = 'YES' ] || [ "$3" = 'yes' ] || [ "$3" = 'Y' ] || [ "$3" = 'y' ] ; then
        ...
        echo "\$3 must be 'yes' or 'no'!"
        echo "abort."
        exit
    fi
    
else
    echo "Usage : bash xselect-basic_rsl_v${VERSION}.bash BASENAME cleanevtfile (CLOBBER_OUTFILE; optional)"
    echo "    BASENAME : Stem name of outputfile"
    echo "    cleanevtfile : A clean event file of RESOLVE"
    echo "    CLOBBER_OUTFILE (Optional) : Whether overwrite outfiles"
    exit
fi
