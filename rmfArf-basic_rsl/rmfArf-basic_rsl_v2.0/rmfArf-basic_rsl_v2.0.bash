...
if [ $# -eq 5 ] ; then
    ...
    echo "    \$3 ehkfile"
    echo "    \$4 exp gti file"
    echo "    \$5 center calculation method (DEFAULT or SCRIPT)"
    echo ""
    echo "    Outfile"
    echo "      ... exposure map file"
    echo "      ... energy redistribution matrix file (RMF)"
    echo "      ... ancillary response function (ARF) file"
    exit
fi
...
echo "center_calc_method(\$5) = ${center_calc_method}" >> $f_log
...
xaarfgen \
    ...
    regmode=DET \
    regionfile=${HEADAS}/refdata/region_RSL_det.reg \
    sourcetype=POINT \
